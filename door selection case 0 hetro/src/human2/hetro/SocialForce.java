...
//    static double Ymax = worldYSize;         // Maximum y point of terrain
    ...
    // /////////////////////////////////////////////////////////////////////////
    private Vector2d forceBound(Vector2d v, Human agent ) {
     

        if ( v.length()> agent.MAX_VELOCITY) {
            v.normalize();
            v.scale(agent.MAX_VELOCITY);
            //x = var;
        } 
...
//            System.out.println("Case Normal; ");
    ...
    public static void initializeObstacleSet(List<Obstacle> xmlObstacleList) {
      ...
      //          vertexX[i] = vertices.get(i).getX();								//change by Ashu, remove the vivid sol import later
    ...
    // /////////////////////////////////////////////////////////////////////////
    @Override
    public Vector2d calculateVelocity(Human me, Bag neighbors, Bag obses,
        ...
        double Vxi =0;// me.getVelocity().getX();           // X-velocity
        double Vyi =0;// me.getVelocity().getY();           // Y-velocity
        double Ri = me.getRadius();                     // Agent's radius
        double Mi = 4.0  ;                     // Agent's mass
        ...
        for (int i = 0; i < neighbors.size(); i++) {
            ...
            Human tempAgent = (Human) neighbors.get(i);
            ...
            double Vxj =0;//= tempAgent.getVelocity().getX();
            double Vyj =0;//= tempAgent.getVelocity().getY();
