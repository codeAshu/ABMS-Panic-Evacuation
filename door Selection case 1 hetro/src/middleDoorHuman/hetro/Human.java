...
public class Human implements Steppable {
	...
	double visibility = (Crowd.yMaxB - Crowd.yMinB)/2;  //visibility of the door
	...
	private Vector2D getGoal(Crowd bd) {
		...
		if(huDistupper <= visibility)
			upper = true;
		if(huDistlower <= visibility)
			lower = true;
	...
	public void step(final SimState state) 
	{
		...
		if(!this.dead)
		{
			...
			//assign goal
			vel = vel.add(getGoal(bd).amplify(goalMultiplier));				              	  //G multiplier
	...
	private void calculateMultipliers() {
		...
		if ( ( (Math.abs(HumanPosition.x -Crowd.doorLeft) <30) ||(Math.abs(HumanPosition.x -Crowd.doorRight)<30 ) ) 
			...
			goalMultiplier *= 7.5;															//G multiplier
		...
		for( int i = 0 ; i < nearbyHuman.numObjs ; i++ )
		{
...

			{
				...
				if( ( human.getRadius()+p.getRadius() )  > Math.sqrt(dist))
				{
					Vector2D temp = HumanPosition.subtract(new Double2D(p.x,p.y));
					temp = temp.normalize();
					velocity = velocity.add(temp).amplify(0.20);	//return false;				
				}	
				//	return false;				
			}
		}
