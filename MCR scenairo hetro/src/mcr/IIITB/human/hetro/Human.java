...
public class Human implements Steppable {
	...
	public double mass ;
	public  double CENTROID_DISTANCE = 2*diameter ;     //diameter was fixed to 3 so
	public  double AVOID_DISTANCE = diameter+2;
	public  double COPY_SPEED_DISTANCE = 2*diameter;
	public static  double ADJUSTMENT_RATE = 0.05;
	public static int trigger = 0;
	public static double MAX_DISTANCE = 10;
	
	//	public static final double ADJUSTMENT_RATE = 0.05;
	...
	public  double UpperVisibility = 10*diameter;  //visibility of the upper door  
	public  double lowerVisibility=40*diameter;   //visibility of the  lower door
	...
	Bag nearbyHuman = new Bag();
	Bag nearbyObst = new Bag();
	double[] distSqrTo;
	public double x;
	public double y;
	Bag hum;
	...
	public Vector2D towardsFlockCenterOfMass( final Crowd state )
	{
		...
		for( int i = 0 ; i < nearbyHuman.numObjs ; i++ )
		{
			if( nearbyHuman.objs[i] != this &&
					distSqrTo[i] <= this.CENTROID_DISTANCE * this.CENTROID_DISTANCE &&
					distSqrTo[i] > this.AVOID_DISTANCE * this.AVOID_DISTANCE )
			{
				Human p = (Human)(nearbyHuman.objs[i]);
	...
	public Vector2D matchFlockSpeed( final SimState state )
	{
		...
		{
			if( nearbyHuman.objs[i] != this &&
					distSqrTo[i] <= this.COPY_SPEED_DISTANCE *this.COPY_SPEED_DISTANCE &&
					distSqrTo[i] > this.AVOID_DISTANCE * this.AVOID_DISTANCE )
			{
	...
	public Vector2D avoidObstacles( final SimState state )
	{
		...
		for( int i = 0 ; i < info.size() ; i++ )
		{
			...
			if( dist <= info.get(i).get(0)/2+this.AVOID_DISTANCE )
			{
	...
	private Vector2D getExit(Crowd bd) {
		...
		double midLow = (Crowd.lwDoorLeft + Crowd.lwDoorRight)/2;
		...
		if(upper)
		{
			...
			else
			{
				...
				yGoal = (int) (Crowd.yMinB-bd.random.nextDouble());

				goal = goal.add(( new Vector2D(Crowd.doorLeft+bd.random.nextInt(doorLen)-0.5 , 
						yGoal-(Crowd.yMinB-Crowd.YMIN-10) ) )) ;
			}
		}
		if(lower)
		{
			//give lower door as goal
			yGoal = (int) (Crowd.yMaxB+bd.random.nextDouble());

			goal = goal.add(( new Vector2D(Crowd.lwDoorLeft+bd.random.nextInt(doorLen)-0.5 , 
					yGoal+(Crowd.YMAX-Crowd.yMaxB-10) ) ));
		}
	...
	private Vector2D getGoal(Crowd bd) {
		...
		//which area you are in?

		if( (    
				(HumanPosition.x>60 && HumanPosition.x<70 && HumanPosition.y>70) ||
				(HumanPosition.x>140 && HumanPosition.x<170) ||
				(HumanPosition.x>182 && HumanPosition.x<194 && HumanPosition.y<90)  ))
		{
			goal = goal.add(new  Double2D(HumanPosition.x+bd.random.nextInt(25)-bd.random.nextInt(25),
					60+bd.random.nextInt( (int)Crowd.yMaxB-130)));

		}
		
		//if in any of the passage come to front area	
		if( (   (HumanPosition.x>80 && HumanPosition.x<90) ||
				(HumanPosition.x>120 && HumanPosition.x<140) ||
				(HumanPosition.x>170 && HumanPosition.x<182)  )
				&& (HumanPosition.y <120))
		{
			goal = goal.add(new  Double2D(HumanPosition.x+bd.random.nextInt(2)-bd.random.nextInt(2),
					100+bd.random.nextInt( (int)Crowd.yMaxB-130)));

		}
		//if in front area go to any door which is closer
		if( (HumanPosition.y>120 && HumanPosition.y <Crowd.yMaxB ) 
				&&(HumanPosition.x > 120) )
		{
			//give lower door as goal
			yGoal = (int) (Crowd.yMaxB-bd.random.nextInt((int) (15*this.diameter) ));
			//velocity = velocity.amplify(0.5);
			goal = goal.add(( new Vector2D(Crowd.lwDoorLeft-bd.random.nextInt((int)(3*this.diameter))-0.5 , 
					yGoal ) ));
		}
		else
		{
			//give upper door as goal go left side
			yGoal = (int) (Crowd.yMinB+bd.random.nextInt((int)(10*this.diameter) ));

			//velocity = velocity.amplify(0.5);
			goal = goal.add(( new Vector2D(Crowd.doorLeft+bd.random.nextInt((int)(3*this.diameter))-0.5 , 
					yGoal  )) );
		}

		//if in leftmost or rightmost passage , go to the door directly
	...
	public void step(final SimState state) 
	{
		...
		{	//faster way to access the agent
			...
			this.AVOID_DISTANCE = this.diameter+2;
			this.CENTROID_DISTANCE = this.diameter*2;
			this.COPY_SPEED_DISTANCE = this.diameter*2;

			this.UpperVisibility = this.diameter*20;
			this.lowerVisibility = this.diameter*40;

			preprocessHumans( bd, temp, MAX_DISTANCE );
			preprocessObstacles(bd, temp, MAX_DISTANCE);
		}
		...
		vel = vel.add( getExit(bd).amplify(4.5));


		//if you are near the exit and you can see so more eager to go to the goal
		if (
				( (Math.abs(HumanPosition.x -Crowd.doorRight)<5*this.diameter)  && (HumanPosition.y-Crowd.yMinB )<20*this.diameter )
				||
				( (Math.abs(Crowd.lwDoorLeft-HumanPosition.x ) <10*this.diameter) &&(Crowd.yMaxB-HumanPosition.y)<20*this.diameter)  
				)
		{
			vel = vel.add( getExit(bd).amplify(8.5));
		}
		...
		vel = vel.add( matchFlockSpeed(bd).amplify(2.5) );


		//if close to door get away from close by human by a large factor
		if (
				( (Math.abs(HumanPosition.x -Crowd.doorRight)<20 )  && (HumanPosition.y-Crowd.yMinB )<15   )
				||
				( (Math.abs(HumanPosition.x -Crowd.lwDoorLeft) <20) &&(Crowd.yMaxB-HumanPosition.y)<15)  
				)												  
	...
	private  void calculatePanic(Crowd bd, Human human) {
		...
		double huDistlower = Math.sqrt( distanceSquared(lowerDoor, HumanPosition)  );
			...
			if(huDistupper< huDistlower)
				panicDistance = Math.sqrt(distanceSquared(HumanPosition, upperDoor));
			else
				panicDistance = Math.sqrt(distanceSquared(HumanPosition, lowerDoor));
	...
	public boolean isValidMove( final Crowd crowd, Human human, final Double2D newLoc)
	{
		...
		if(
				( (newLoc.x> Crowd.doorLeft-human.diameter && newLoc.x< Crowd.doorRight+human.diameter)
						&& (y < Crowd.yMinB+7 ) )
						|| 
						( (newLoc.x> Crowd.lwDoorLeft-human.diameter && newLoc.x< Crowd.lwDoorRight+human.diameter)
								&&(y >Crowd.yMaxB-7) ) //for two doors!!
				)
			return true;
	...
	public void calculateSocialForce()
	{
		...
		for (int i = 0; i < injuryFromHuman.numObjs; i++) {
			...
			double Bj = 0.44;        //0.08*5.5           			 //Published value = 0.08 m
			double kj = 12000;       //12000;     			 //Published value = 120000 kg/s^2
			double kappaj = 4363;   //24000/6;     			 //Published value = 240000 kg/ms
