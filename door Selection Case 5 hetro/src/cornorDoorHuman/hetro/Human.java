...
public class Human   implements Steppable {
	...
	private Vector2D getGoal(Crowd bd) {
		...
		if(huDistupper <= this.UpperVisibility)  //every human has different visibility slightly
			upper = true;
		...
		if(upper)
		{
			if(HumanPosition.y <Crowd.yMinB-10)
			{	 
		...
		if(lower)
		{
			if(HumanPosition.y >Crowd.yMaxB+10)
			{	 
				Crowd.HumansEnvironment.remove(this);
			}
			else
	...
	public void step(final SimState state) 
	{
		...
		{	//faster way to access the agent
			...
			this.UpperVisibility =(int)(Crowd.yMaxB-Crowd.yMinB);
			this.lowerVisibility = (int)(Crowd.yMaxB-Crowd.yMinB);
		...
		if(!this.dead)
		{
			...
			if (
					( (Math.abs(HumanPosition.x -Crowd.doorRight)<30 )  && (HumanPosition.y-Crowd.yMinB )<20 )
					||
					( (Math.abs(HumanPosition.x -Crowd.lwDoorLeft) <30) &&(Crowd.yMaxB-HumanPosition.y)<20)  
					)
			...
			if(   
					( (HumanPosition.x>Crowd.doorLeft && HumanPosition.x<Crowd.doorRight) 
							&&( (HumanPosition.y-Crowd.yMinB )<5 )	)	
							||		
							( (HumanPosition.x>Crowd.lwDoorLeft && HumanPosition.x<Crowd.lwDoorRight)  
									&&( (Crowd.yMaxB-HumanPosition.y)<5) ) 
					)
